...
#pragma region Private MetaMethod
	...
	void pushVariable(lua_State* L, Idx<Variable> variable) {
		...
		*p = variable;
		lua_pushstring(L, DEBUGGER_META);
	...
	int createNode(lua_State* L) {
		auto arenaRef = DebuggerFacade::Get().GetDebugger(L)->GetVariableArena();
		if (arenaRef) {
	...
	int cplHelperInit(lua_State* L) {
		lua_getglobal(L, ExtensionPoint::extensionTable);
		if (lua_istable(L, -1)) {
...
#pragma endregion
	...
	void ExtensionPoint::Initialize(lua_State* L)
	{
		...
		lua_pushstring(L, "cplHelperInit");
		lua_pushcfunction(L, cplHelperInit);
		lua_rawset(L, -3);
		lua_pop(L, 1);
	...
	bool ExtensionPoint::QueryVariableGeneric(lua_State* L, Idx<Variable> variable, const char* typeName, int object, int depth, const char* queryFunction)
	{
		...
		if (lua_istable(L, -1)) {
			...
			if (lua_isfunction(L, -1)) {
				...
				if (r == LUA_OK) res = lua_toboolean(L, -1);
				else {
